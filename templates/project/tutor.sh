...
step1() {
  title "第 1 步 · 认识工作台"
  echo "  这是一套「导演 + 写手」的 AI 小说写作系统，全部跑在你自己电脑上。"
  echo "  平时只需要记住一个入口："
  cmd "./workbench.sh"
  echo ""
  echo "  工作台里 9 个工具分别是："
  ...
  printf "  %-16s %s\n" "novel_flow.sh"   "推演+成章一条龙：角色自主演 → 提炼分镜 → 成章"
  printf "  %-16s %s\n" "novel_pipeline.sh" "一键全流程（含推演引擎拉起）"
  printf "  %-16s %s\n" "novel_factory.sh" "短篇工厂：一句话写一本短篇小说"
  printf "  %-16s %s\n" "sync_sodarie.sh" "把章节同步进 Sodarie Novel 图形界面"
  printf "  %-16s %s\n" "nest_drama_run.sh" "推演引擎管理（群像自主演绎）"
  printf "  %-16s %s\n" "workbench.sh"    "总入口菜单，上面全部从这里进"
  echo ""
...
step5() {
  title "第 5 步 · 看结果"
  echo "  写完后，成果会在三个地方出现："
  echo "    1. chapters/chXXX-标题/       章节文件（项目内）"
  echo "    2. ~/Desktop/小说章节/         自动同步到桌面（方便查看）"
  echo "    3. Sodarie Novel GUI          图形界面（跑 sync_sodarie.sh 同步）"
  echo ""
